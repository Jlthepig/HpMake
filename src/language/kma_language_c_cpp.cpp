...
void PreCheck(GlobalData& globalData)
{
	...
	if (globalData.targetProfile.unityBatch != 0
		&& !ContainsValue(globalData.targetProfile.customFlags, CustomFlag::F_UNITY_BUILD))
	{
		KalaMakeCore::CloseOnError(
			"LANGUAGE_C_CPP",
			"Field 'unitybatch' cannot be used without the 'unity-build' custom flag!");
	}
	...
	auto compile = [&isMSVC, &frontArg, &globalData]() -> vector<path>
		{
			...
			auto needs_compile = [&newestHeaderTime](
				const vector<path>& originalSources,
				const path& object
				) -> bool
				{
					...
					if (objTime < kmakeTime) return true;
					if (objTime < newestHeaderTime) return true;

					for (const auto& src : originalSources)
					{
						if (objTime < last_write_time(src)) return true;
					}
			...
			vector<path> finalCompileSources{};
			vector<vector<path>> unityBatches{};

			bool isUnityBuild = ContainsValue(globalData.targetProfile.customFlags, CustomFlag::F_UNITY_BUILD);

			if (isUnityBuild)
			{
				Log::Print(
					"Starting Unity Build generation.",
					"LANGUAGE_C_CPP",
					LogType::LOG_INFO);

				path unityDir = globalData.targetProfile.buildPath / "unity";
				if (!exists(unityDir))
				{
					string errorMsg = CreateNewDirectory(unityDir);
					if (!errorMsg.empty())
					{
						KalaMakeCore::CloseOnError(
							"LANGUAGE_C_CPP",
							"Failed to create new unity dir for compilation! Reason: " + errorMsg);
					}
				}

				const vector<path>& sources = globalData.targetProfile.sources;
				
				size_t batchSize = globalData.targetProfile.unityBatch == 0 ? 8 : globalData.targetProfile.unityBatch;

				for (size_t i = 0; i < sources.size(); i += batchSize)
				{
					size_t end = min(i + batchSize, sources.size());
					
					string ext = ".c";
					for (size_t j = i; j < end; ++j)
					{
						string e = sources[j].extension().string();
						if (e == ".cpp" || e == ".cc" || e == ".cxx")
						{
							ext = ".cpp";
							break;
					...
					path unityFile = unityDir / ("unity_" + to_string(i / batchSize) + ext);
					
					ofstream out(unityFile);
					if (!out.is_open())
					{
						KalaMakeCore::CloseOnError(
							"LANGUAGE_C_CPP",
							"Failed to create unity file '" + unityFile.string() + "'!");
					}

					out << "// Auto-generated by KalaMake Unity Build\n";
					
					vector<path> batchSources{};
					for (size_t j = i; j < end; ++j)
					{
						string p = sources[j].string();
						replace(p.begin(), p.end(), '\\', '/');
						out << "#include \"" << p << "\"\n";
						batchSources.push_back(sources[j]);
					}
					out.close();

					finalCompileSources.push_back(unityFile);
					unityBatches.push_back(batchSources);
				}

				Log::Print(
					"Finished Unity Build generation! Created " + to_string(finalCompileSources.size()) + " unity file(s).\n",
					"LANGUAGE_C_CPP",
					LogType::LOG_SUCCESS);
			}
			else
			{
				finalCompileSources = globalData.targetProfile.sources;
				for (const auto& s : finalCompileSources)
				{
					unityBatches.push_back({ s });
				}
			}
			...
			auto compile_file = [
				...
				{
					const path& s = finalCompileSources[targetIndex];
					...
					if (needs_compile(unityBatches[targetIndex], objPath))
					{
						Log::Print(
			...
			if (finalCompileSources.size() == 1) compile_file(0);
			else
			{
				u16 max_jobs = min(
					scast<size_t>(globalData.targetProfile.jobs), 
					finalCompileSources.size());
					...
					workers.emplace_back([
						...
						compile_file] 
						{
							while (true)
							{
								...
								if (scast<size_t>(idx) >= finalCompileSources.size()) break;
